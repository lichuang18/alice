...
static void update_general_status(struct f2fs_sb_info *sbi)
{
	...
	// lch
	si->mot2_update_path_invalid_blks_total =
	atomic64_read(&sbi->mot2.update_path_invalid_blks_total);

	si->mot2_queue_gen_blks_total =
		atomic64_read(&sbi->mot2.queue_gen_blks_total);
...
static int stat_show(struct seq_file *s, void *v)
{
	...
	list_for_each_entry(si, &f2fs_stat_list, stat_list) {
		...
		// lch
		seq_printf(s, "MOT2 update_path_invalid_blks_total: %llu\n",
	   		si->mot2_update_path_invalid_blks_total);

		seq_printf(s, "MOT2 queue_gen_blks_total: %llu\n",
			si->mot2_queue_gen_blks_total);
...
{
#ifdef CONFIG_DEBUG_FS
	f2fs_debugfs_root = debugfs_create_dir("cf2fs", NULL);
