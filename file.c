...
static int f2fs_ioc_fitrim(struct file *filp, unsigned long arg)
{
	...
	printk(KERN_INFO "f2fs_ioc_fitrim: ENTER\n");

	if (!capable(CAP_SYS_ADMIN)) {
		printk(KERN_INFO "f2fs_ioc_fitrim: no CAP_SYS_ADMIN\n");
		return -EPERM;
	}

	if (!f2fs_hw_support_discard(F2FS_SB(sb))) {
		printk(KERN_INFO "f2fs_ioc_fitrim: hw not support discard\n");
		return -EOPNOTSUPP;
	}
	...
	printk(KERN_INFO "f2fs_ioc_fitrim: calling f2fs_trim_fs, minlen=%u\n",
		...
		pgoff_t off, block_t block, block_t len, u32 flags)
{
	...
	if (flags & F2FS_TRIM_FILE_DISCARD)
		pr_info("F2FS: issuing discard: sector=%llu, nr_sects=%llu, flags=%x\n", sector, nr_sects,
